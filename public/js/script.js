...
// const publicIp='http://13.126.190.153:4000';
  ...
  try {
    ...
    async function displayExpenses() {
      ...
      const res = await axios.get(`${publicIp}/admin/getAllExpenses`, {
        ...
        params: { page: currentPage, pageLimit },
      });
      ...
      document.getElementById("showingRange").innerHTML = totalPages;
...
async function showexpenses(expenseItem) {
  try {
    ...
    const html = `
      <tr id="${expenseItem._id}" class="text-gray-600 text-sm hover:bg-gray-100">
     ...
     <td class="py-2  border-b border-gray-400">
      <button onclick="editexpense('${expenseItem._id}','${expenseItem.amount}','${expenseItem.description}','${expenseItem.category}','${expenseItem.amountType}')" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-3 rounded "><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4">
